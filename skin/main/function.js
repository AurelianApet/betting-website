...
function teamBeforeAddMember(){
	...
	if(!this.referee.value){winjinAlert("没有推荐入用户名","alert");return false;}
	if(!/^\w{4,16}$/.test(this.referee.value)){winjinAlert("推荐入用户名由4到16位的字母或数字组成","alert");return false;}
...
function teamAddMember(err, data){
	if(err){
		...
		$('#referee').val(this.referee.value);
		winjinAlert(data,"ok");
	}
}
function dataAddCode(){
