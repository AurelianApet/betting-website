...
class User extends WebBase{
	...
	public final function loginedto(){
		...
		$referee=wjStrFilter($_POST['referee']);
	...
	public final function vcode($rmt=null){	
		...
		$width=130;
		$height=40;
		$img=new CImage($width, $height, 26);
	...
	public final function registered(){
		...
		$parentId = $this->getValue("select uid from {$this->prename}members where username=?", $referee);
		...
		if(!ctype_alnum($parentId)) throw new Exception('推荐人用户名包含非法字符');
//		if(!ctype_alnum($parentId)) throw new Exception($parentId);
